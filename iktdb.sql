...
CREATE TABLE sizing_sets (
    ...
    bill_no VARCHAR(50),
    amount DECIMAL(14,2),
    total_end DECIMAL(14,2),
    sizing_mtr DECIMAL(14,3),
    sizing_received_khard DECIMAL(14,3),
    sizing_fresh_yarn_received DECIMAL(14,3),
    balance_in_sizing DECIMAL(14,3),
    sizing_consumption DECIMAL(14,3),
...
-- =========================================================
-- 10. SIZING SET YARN LINES (ALLOCATIONS)
-- =========================================================

CREATE TABLE sizing_set_yarn_lines (
    yarn_line_id BIGINT PRIMARY KEY AUTO_INCREMENT,

    sizing_set_id BIGINT NOT NULL,
    sr_no INT,
    source_from VARCHAR(40),
    fresh_winding VARCHAR(20),
    count_id BIGINT,
    tickit_id BIGINT,
    bags DECIMAL(12,3),
    cones DECIMAL(12,3),
    ...
    CONSTRAINT fk_yarn_line_set
        FOREIGN KEY (sizing_set_id)
        REFERENCES sizing_sets(sizing_set_id)
        ON DELETE CASCADE,

    CONSTRAINT fk_yarn_line_count
        FOREIGN KEY (count_id)
        REFERENCES yarn_counts(count_id),

    CONSTRAINT fk_yarn_line_tickit
        FOREIGN KEY (tickit_id)
        REFERENCES tickits(tickit_id),

    CONSTRAINT fk_yarn_line_inward
        FOREIGN KEY (yarn_inward_id)
        REFERENCES yarn_inward(yarn_inward_id)
);
